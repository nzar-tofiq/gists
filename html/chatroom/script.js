...
window.addEventListener("load", function() {
	...
	document.getElementById("registerForm").addEventListener("submit", function(){
		...
		if (pass1 === pass2 && pass1 !== "" && userName !== "") {
			...
			document.getElementById("feedback").innerHTML = "Registration failed - passwords don't match!";
		}	
	});
				...
				if (xhLogIn.readyState === 4) {
					if(xhLogIn.status === 200){
						if(xhLogIn.responseText === "ok"){
							...
							document.getElementById("login-failed").style.display = "inline-block";
			...
			if (xhr.readyState === 4) {
				if (xhr.status === 200) {
					...
					for (var i = 0; i < msgs.length; i++) {
						...
						para.innerHTML = "<span class='usernameMsg'>"+ msgFrom +"</span> says:" + "<br/> <span class='msgContent'>"+contentTxt+"</span> <br/>Sent at: "+msgTime+"<br/>";
			...
			if (xhUserList.readyState === 4) {
				if (xhUserList.status === 200) {
					...
					for (var i = 0; i <  xmlResp.getElementsByTagName("user").length; i++) {
						...
						para.appendChild(document.createTextNode(userlog));
						//para.appendChild("p");
						console.log("User: "+user+"Last-seen: "+lastseen);
						membersBox.appendChild(para);
						para.appendChild(br);
					}
					setTimeout(getUsers, 10*60*50);
				}
			} 
		};
	}else{
